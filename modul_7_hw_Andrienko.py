...
def main(path):
    ...
    # перемфщує невідомі файлу у спеціальну папку
    ...
    def decode_folder(dst_arh):  # розпаковуємо архіви
        python_arch_list = ['\.zip$', '\.tar\.gz$',
                            '\.tar\.bz2$', '\.tar\.xz$', '\.tar\.Z$', '.tar$']
        path = dst_arh
...
# В блоці if __name__ == '__main__': зробіть запуск main без параметрів. 
# Це будє потрібно для наступного завдання
